...
- 修正电网设备估值板块误指向“电池”的配置，并将三花智控、华宏科技、再升科技、超声电子移入符合主营业务的模型族；新增锂资源与特种纸模型覆盖。
- PE/PS/PB 改为独立公允锚；只对有效独立镜头计算分歧，低于配置权重覆盖门槛时返回 `LOW_MODEL_COVERAGE / NO_TRADE`。
- 归母净利润成为 PEG/PE/EPS/ROE 的统一口径；总股本优先按总市值/价格，EPS/SPS/BVPS 从同一股本闭合。
- 板块估值也使用多镜头组合；聚合 OCF/利润得到现金转化，并输出每个模型的配置权重、有效权重和丢弃原因。
- 快照加入模型、配置与板块成分哈希，旧历史自动隔离；交易日使用 XSHG 日历，行情日期与估值历史日期分离。
- 增加低基数/扭亏标记、bootstrap 过期和显式权重、模型先验交易带限仓、主线全局缺失告警及模型化数据质量权重。
