...
脚本默认扫描行业和概念源全集、不应用概念过滤规则。缓存保存在 Colab 会话本地磁盘（`/content/a-share-mainline-cache`），会话结束后会被清空；表格和两张图片直接显示在 Colab 输出区域，不创建下载任务。日线按“东方财富 → 同花顺 → 申万研究一级/二级行业”回退；真实主力资金不可用时会显示明确标注的 CMF 量价代理。脚本按 Noto CJK 字体文件绝对路径注册中文字体；若仍遇到限流，把 `run_scan.py` 顶部的 `WORKERS` 改为 `1` 后重跑。
